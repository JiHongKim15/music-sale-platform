...
import { ArrowLeft, Heart, Share2, Eye, Clock, CreditCard, ShoppingCart, MessageCircle, Truck, Shield, AlertCircle } from 'lucide-react';
...
export function InstrumentDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
        <div className="text-center">
          ...
          <p className="text-orange-600">상품 정보를 불러오는 중...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
        <div className="text-center">
          ...
          <button
            onClick={() => navigate(-1)}
            className="inline-flex items-center px-6 py-3 bg-orange-500 text-white rounded-full hover:bg-orange-600 transition-all duration-300 hover:scale-105 font-medium text-sm"
          >
  ...
  const getConditionText = (condition: ProductCondition, grade?: ProductConditionGrade | null) => {
    if (condition === ProductCondition.NEW) return '새 제품';
    return `중고${grade ? ` • ${grade}등급` : ''}`;
  };

  const getConditionColor = (condition: ProductCondition, grade?: ProductConditionGrade | null) => {
    if (condition === ProductCondition.NEW) return 'bg-green-100 text-green-700';
    
    switch (grade) {
      case ProductConditionGrade.S: return 'bg-blue-100 text-blue-700';
      case ProductConditionGrade.A: return 'bg-green-100 text-green-700';
      case ProductConditionGrade.B: return 'bg-yellow-100 text-yellow-700';
      case ProductConditionGrade.C: return 'bg-orange-100 text-orange-700';
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-3 mb-6">
                      <span className={`px-4 py-2 rounded-full text-sm font-medium ${getConditionColor(product.condition, product.conditionGrade)}`}> 
                        {getConditionText(product.condition, product.conditionGrade)}
                      </span>
